...
public class ConsultantCommerceMergeAddCartStrategy extends AbstractCommerceAddToCartStrategy
{
	/**
	 * Adds an item to the cart for pickup in a given location
	 * Modify the addToCart strategy for validation the consultant 
	 * model with StockLevel is 1
	 * @param CommerceCartParameter parameter
	...
	public CommerceCartModification addToCart(final CommerceCartParameter parameter) throws CommerceCartModificationException
	{
		...
		try
		{
			...
			//It already add into session cart previous
			if (actualAllowedQuantityChange == cartLevel && actualAllowedQuantityChange == 0)
			{
				actualAllowedQuantityChange = 1;
